...
func newTestService(t *testing.T) (
	...
	*authz.MockService,
	...
	svc := NewService(blockRepo, authzSvc).(*service)
	return svc, blockRepo, authzSvc
}
...
func TestBlock_StaffTargetsRejected(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			// given
			svc, _, authzSvc := newTestService(t)
			blocker := uuid.New()
			target := uuid.New()
...
func TestBlock_OK(t *testing.T) {
	// given
	svc, blockRepo, authzSvc := newTestService(t)
	blocker := uuid.New()
	target := uuid.New()
...
func TestUnblock_OK(t *testing.T) {
	// given
	svc, blockRepo, _ := newTestService(t)
	blocker := uuid.New()
	target := uuid.New()
