...
func newTestService(t *testing.T) (*service, *testMocks) {
	...
	svc := NewService(chatRepo, userRepo, roleRepo, vanityRoleRepo, banRepo, bannedWordRepo, auditRepo, authzSvc, notifSvc, blockSvc, uploadSvc, settingsSvc, mediaProc, hub, livekitSvc, contentfilter.New()).(*service)
...
func TestCreateGroupRoom_InvalidKind(t *testing.T) {
	// given
	svc, _ := newTestService(t)
	creator := uuid.New()
	req := dto.CreateGroupRoomRequest{Name: "Room", ChannelKind: "bogus"}

	// when
	_, err := svc.CreateGroupRoom(context.Background(), creator, req)

	// then
	require.ErrorIs(t, err, ErrInvalidChannelKind)
}
...
func TestCreateGroupRoom_CreateRoomError(t *testing.T) {
	...
	m.chatRepo.EXPECT().CreateRoom(mock.Anything, mock.Anything, "Room", "", "group", true, false, "text", creator).Return(errors.New("boom"))
...
func TestCreateGroupRoom_AddTagsError(t *testing.T) {
	...
	m.chatRepo.EXPECT().CreateRoom(mock.Anything, mock.Anything, "Room", "", "group", true, false, "text", creator).Return(nil)
...
func TestCreateGroupRoom_TextKind_OK(t *testing.T) {
	// given
	svc, m := newTestService(t)
	creator := uuid.New()
	req := dto.CreateGroupRoomRequest{Name: "Room"}
	...
	m.chatRepo.EXPECT().GetRoomByID(mock.Anything, mock.Anything, creator).Return(&repository.ChatRoomRow{Name: "Room", Type: "group", ChannelKind: "text"}, nil)
	m.chatRepo.EXPECT().GetRoomMembers(mock.Anything, mock.Anything).Return([]uuid.UUID{creator}, nil)
	...
	assert.Equal(t, "text", got.ChannelKind)
}

func TestCreateGroupRoom_VoiceKind_OK(t *testing.T) {
	// given
	svc, m := newTestService(t)
	creator := uuid.New()
	req := dto.CreateGroupRoomRequest{Name: "Room", ChannelKind: "voice"}
	m.chatRepo.EXPECT().CreateRoom(mock.Anything, mock.Anything, "Room", "", "group", true, false, "voice", creator).Return(nil)
	m.chatRepo.EXPECT().AddMemberWithRole(mock.Anything, mock.Anything, creator, "host", false).Return(nil)
	m.chatRepo.EXPECT().GetRoomByID(mock.Anything, mock.Anything, creator).Return(&repository.ChatRoomRow{Name: "Room", Type: "group", ChannelKind: "voice"}, nil)
	m.chatRepo.EXPECT().GetRoomMembers(mock.Anything, mock.Anything).Return([]uuid.UUID{creator}, nil)
	m.userRepo.EXPECT().GetByID(mock.Anything, creator).Return(sampleUser(creator), nil)
	...
	require.NoError(t, err)
	require.NotNil(t, got)
	assert.Equal(t, "voice", got.ChannelKind)
}
...
func TestSetRoomMuted_GetRoomError(t *testing.T) {
	// given
	svc, m := newTestService(t)
	roomID := uuid.New()
	userID := uuid.New()
...
func TestSetRoomMuted_SystemRoomNonStaff(t *testing.T) {
	// given
	svc, m := newTestService(t)
	roomID := uuid.New()
	userID := uuid.New()
	m.chatRepo.EXPECT().GetRoomByID(mock.Anything, roomID, userID).Return(&repository.ChatRoomRow{ID: roomID, IsSystem: true}, nil)
...
func TestSetRoomMuted_SetMutedError(t *testing.T) {
	...
	roomID := uuid.New()
	userID := uuid.New()
	m.chatRepo.EXPECT().GetRoomByID(mock.Anything, roomID, userID).Return(&repository.ChatRoomRow{ID: roomID}, nil)
	m.chatRepo.EXPECT().EnsureMember(mock.Anything, roomID, userID).Return(nil)
...
func TestListRooms_RepoError(t *testing.T) {
	...
	userID := uuid.New()
	m.authzSvc.EXPECT().GetRole(mock.Anything, userID).Return(role.Role("user"), nil)
	m.chatRepo.EXPECT().ListAllChannels(mock.Anything, userID, false).Return(nil, errors.New("boom"))
...
func TestListRooms_RoleError(t *testing.T) {
	// given
	svc, m := newTestService(t)
	userID := uuid.New()
	m.authzSvc.EXPECT().GetRole(mock.Anything, userID).Return(role.Role(""), errors.New("boom"))
...
func TestListRooms_OK(t *testing.T) {
	...
	roomID := uuid.New()
	m.authzSvc.EXPECT().GetRole(mock.Anything, userID).Return(role.Role("user"), nil)
	m.chatRepo.EXPECT().ListAllChannels(mock.Anything, userID, false).Return([]repository.ChatRoomRow{{ID: roomID, Type: "group", ChannelKind: "text"}}, nil)
	...
	assert.Equal(t, "text", got.Rooms[0].ChannelKind)
}

func TestListRooms_StaffSeesSystem(t *testing.T) {
	// given
	svc, m := newTestService(t)
	userID := uuid.New()
	m.authzSvc.EXPECT().GetRole(mock.Anything, userID).Return(authz.RoleModerator, nil)
	m.chatRepo.EXPECT().ListAllChannels(mock.Anything, userID, true).Return(nil, nil)
...
func TestGetMessages_SystemRoomNonStaff(t *testing.T) {
	// given
	svc, m := newTestService(t)
	userID := uuid.New()
	roomID := uuid.New()
...
func TestGetMessagesBefore_GetRoomError(t *testing.T) {
	// given
	svc, m := newTestService(t)
	userID := uuid.New()
	roomID := uuid.New()
...
func TestGetMessagesBefore_SystemRoomNonStaff(t *testing.T) {
	// given
	svc, m := newTestService(t)
	userID := uuid.New()
	roomID := uuid.New()
...
func TestSendMessage_NonMemberCanPostToTextChannel(t *testing.T) {
	// given
	svc, m := newTestService(t)
	senderID := uuid.New()
	roomID := uuid.New()
	m.chatRepo.EXPECT().GetRoomByID(mock.Anything, roomID, senderID).Return(&repository.ChatRoomRow{ID: roomID, Type: "group", ChannelKind: "text"}, nil)
	m.chatRepo.EXPECT().EnsureMember(mock.Anything, roomID, senderID).Return(nil)
...
func TestSendMessage_GetRoomError(t *testing.T) {
	// given
	svc, m := newTestService(t)
	senderID := uuid.New()
	roomID := uuid.New()
	m.chatRepo.EXPECT().GetRoomByID(mock.Anything, roomID, senderID).Return(nil, errors.New("boom"))
...
func TestSendMessage_SystemRoomNonStaff(t *testing.T) {
	// given
	svc, m := newTestService(t)
	senderID := uuid.New()
	roomID := uuid.New()
	m.chatRepo.EXPECT().GetRoomByID(mock.Anything, roomID, senderID).Return(&repository.ChatRoomRow{ID: roomID, Type: "group", IsSystem: true}, nil)
	m.authzSvc.EXPECT().GetRole(mock.Anything, senderID).Return(role.Role("user"), nil)
...
func TestSendMessage_TimedOut(t *testing.T) {
	...
	senderID := uuid.New()
	roomID := uuid.New()
	m.chatRepo.EXPECT().GetRoomByID(mock.Anything, roomID, senderID).Return(&repository.ChatRoomRow{ID: roomID, Type: "group"}, nil)
	m.chatRepo.EXPECT().EnsureMember(mock.Anything, roomID, senderID).Return(nil)
...
func TestSendMessage_BlockedByRecipient(t *testing.T) {
	...
	m.blockSvc.EXPECT().GetBlockedIDs(mock.Anything, senderID).Return([]uuid.UUID{otherID}, nil)
...
func TestDeleteChat_DeleteMessagesError(t *testing.T) {
	// given
	svc, m := newTestService(t)
	roomID := uuid.New()
	userID := uuid.New()
...
func TestDeleteChat_DeleteRoomError(t *testing.T) {
	// given
	svc, m := newTestService(t)
	roomID := uuid.New()
	userID := uuid.New()
...
func TestMarkRead_SystemRoomNonStaff(t *testing.T) {
	// given
	svc, m := newTestService(t)
	roomID := uuid.New()
	userID := uuid.New()
...
func TestSetRoomNickname_SystemRoomNonStaff(t *testing.T) {
	// given
	svc, m := newTestService(t)
	roomID := uuid.New()
	userID := uuid.New()
...
func TestSetRoomNickname_GetRoomError(t *testing.T) {
	// given
	svc, m := newTestService(t)
	roomID := uuid.New()
	userID := uuid.New()
	m.chatRepo.EXPECT().GetRoomByID(mock.Anything, roomID, userID).Return(nil, errors.New("db"))
...
func TestSetRoomAvatar_SystemRoomNonStaff(t *testing.T) {
	// given
	svc, m := newTestService(t)
	roomID := uuid.New()
	userID := uuid.New()
...
func TestClearRoomAvatar_SystemRoomNonStaff(t *testing.T) {
	// given
	svc, m := newTestService(t)
	roomID := uuid.New()
	userID := uuid.New()
...
func TestListPinnedMessages_HappyPath(t *testing.T) {
	...
	viewerID := uuid.New()
	msgID := uuid.New()
	m.chatRepo.EXPECT().GetRoomByID(mock.Anything, roomID, viewerID).Return(&repository.ChatRoomRow{ID: roomID}, nil)
...
func TestListPinnedMessages_SystemRoomNonStaff(t *testing.T) {
	// given
	svc, m := newTestService(t)
	roomID := uuid.New()
	viewerID := uuid.New()
	m.chatRepo.EXPECT().GetRoomByID(mock.Anything, roomID, viewerID).Return(&repository.ChatRoomRow{ID: roomID, IsSystem: true}, nil)
	m.authzSvc.EXPECT().GetRole(mock.Anything, viewerID).Return(role.Role("user"), nil)
...
func TestListPinnedMessages_GetRoomError(t *testing.T) {
	// given
	svc, m := newTestService(t)
	roomID := uuid.New()
	viewerID := uuid.New()
	m.chatRepo.EXPECT().GetRoomByID(mock.Anything, roomID, viewerID).Return(nil, errors.New("db"))
...
func TestAddReaction_HappyPath(t *testing.T) {
	...
	m.chatRepo.EXPECT().IsMuted(mock.Anything, roomID, authorID).Return(false, nil).Maybe()
...
func TestAddReaction_SystemRoomNonStaff(t *testing.T) {
	// given
	svc, m := newTestService(t)
	messageID := uuid.New()
	roomID := uuid.New()
...
func TestRemoveReaction_SystemRoomNonStaff(t *testing.T) {
	// given
	svc, m := newTestService(t)
	messageID := uuid.New()
	roomID := uuid.New()
