...
func TestAdminListUsers_CustomQuery(t *testing.T) {
	...
	h.ExpectValidSession("valid-cookie", userID)
	h.ExpectHasPermission(userID, authz.PermViewUsers, true)
	ms.EXPECT().ListUsers(mock.Anything, "nightjar", 50, 10).Return(&dto.AdminUserListResponse{}, nil)

	// when
	status, _ := h.NewRequest("GET", "/admin/users?search=nightjar&limit=50&offset=10").
...
func TestAdminGetUser_OK(t *testing.T) {
	...
	h.ExpectValidSession("valid-cookie", userID)
	h.ExpectHasPermission(userID, authz.PermViewUsers, true)
	expected := &dto.AdminUserDetailResponse{AdminUserItem: dto.AdminUserItem{ID: targetID, Username: "nightjar"}}
...
func TestAdminGetSettings_OK(t *testing.T) {
	...
	expected := &dto.SettingsResponse{Settings: map[string]string{"site_name": "Sixth World Sunday"}}
...
func TestAdminUpdateSettings_OK(t *testing.T) {
	...
	h.ExpectValidSession("valid-cookie", userID)
	h.ExpectHasPermission(userID, authz.PermManageSettings, true)
	settings := map[string]string{"site_name": "Sixth World Sunday"}
...
func TestAdminGetVanityRoleUsers_CustomQuery(t *testing.T) {
	...
	h.ExpectValidSession("valid-cookie", userID)
	h.ExpectHasPermission(userID, authz.PermManageVanityRoles, true)
	ms.EXPECT().GetVanityRoleUsers(mock.Anything, "r1", "nightjar", 5, 10).Return(&dto.VanityRoleUsersResponse{}, nil)

	// when
	status, _ := h.NewRequest("GET", "/admin/vanity-roles/r1/users?search=nightjar&limit=5&offset=10").
		WithCookie("valid-cookie").Do()
