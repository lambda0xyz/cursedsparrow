...
func TestCreateGroupRoom_PermissionFailures(t *testing.T) {
	testutil.RunPermissionFailureSuite(t, chatFactory, "POST", "/chat/rooms",
		dto.CreateGroupRoomRequest{Name: "room"}, authz.PermManageChannels)
}
...
func TestCreateGroupRoom_OK(t *testing.T) {
	...
	h.ExpectHasPermission(userID, authz.PermManageChannels, true)
	req := dto.CreateGroupRoomRequest{Name: "room", ChannelKind: "voice"}
	chatMock.EXPECT().CreateGroupRoom(mock.Anything, userID, req).
		Return(&dto.ChatRoomResponse{ID: uuid.New(), Name: "room", ChannelKind: "voice"}, nil)
...
func TestCreateGroupRoom_ServiceErrors(t *testing.T) {
	cases := []struct {
		...
		{"missing fields", chatsvc.ErrMissingFields, http.StatusBadRequest, "channel name is required"},
		{"invalid channel kind", chatsvc.ErrInvalidChannelKind, http.StatusBadRequest, "channel_kind must be"},
		{"internal", errors.New("boom"), http.StatusInternalServerError, "failed to create channel"},
	}
...
func TestDeleteChat_PermissionFailures(t *testing.T) {
	testutil.RunPermissionFailureSuite(t, chatFactory, "DELETE", "/chat/rooms/"+uuid.NewString(), nil, authz.PermManageChannels)
}
...
func TestDeleteChat_ServiceErrors(t *testing.T) {
	cases := []struct {
		...
		{"not found", chatsvc.ErrRoomNotFound, http.StatusNotFound, "channel not found"},
		{"system channel", chatsvc.ErrSystemRoom, http.StatusForbidden, "system channels cannot be deleted"},
		{"internal", errors.New("boom"), http.StatusInternalServerError, "failed to delete channel"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			// given
