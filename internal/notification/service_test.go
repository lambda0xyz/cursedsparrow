...
func TestNotifyMany_IteratesAllParamsAndSwallowsErrors(t *testing.T) {
	...
	paramsList := []dto.NotifyParams{
		{RecipientID: recipient, ActorID: actor, Type: dto.NotifChatRoomMessage, ReferenceID: ref},
		{RecipientID: recipient, ActorID: actor, Type: dto.NotifChatRoomMessage, ReferenceID: ref},
	}
	notifRepo.EXPECT().
		Create(mock.Anything, recipient, dto.NotifChatRoomMessage, ref, "", actor, "").
...
func TestList_OK(t *testing.T) {
	...
	rows := []model.NotificationRow{
		{ID: 1, UserID: userID, Type: dto.NotifReportResolved, ActorUsername: "alice"},
		{ID: 2, UserID: userID, Type: dto.NotifChatMention, ActorUsername: "bob"},
	}
