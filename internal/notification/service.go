...
func isChatRoomNotif(t dto.NotificationType) bool {
	switch t {
	case dto.NotifChatRoomMessage, dto.NotifChatMention, dto.NotifChatReply, dto.NotifChatReaction:
		return true
	default:
		return false
	}
...
func (s *service) Notify(ctx context.Context, params dto.NotifyParams) error {
	...
	if params.Type == dto.NotifChatReaction {
		dup, _ := s.repo.HasRecentDuplicate(ctx, params.RecipientID, params.Type, params.ReferenceID, params.ActorID)
		if dup {
			return nil
		}
	}
