...
type (
	...
	ChatRepository interface {
		CreateRoom(ctx context.Context, id uuid.UUID, name, description, roomType string, isPublic, isRP bool, channelKind string, createdBy uuid.UUID) error
...
func (r *chatRepository) EnsureMember(ctx context.Context, roomID, userID uuid.UUID) error {
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO chat_room_members (room_id, user_id, role) VALUES ($1, $2, 'member')
...
func (r *chatRepository) CountUnreadRoomsForUser(ctx context.Context, userID uuid.UUID) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("count unread rooms: %w", err)
	}
	return count, nil
}
